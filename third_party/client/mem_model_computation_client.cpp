...
void MemModelComputationClient::AnnotateLayerName() {
  ...
  if (start < total_nodes) {
    LTC_LOG(INFO) << "[WARNING] " << total_nodes - start << " leftover nodes are not annotated!";
  }
  // Put a flag for all other nodes
  for (int64_t i = start; i < total_nodes; i ++) {
    node_info_[i].SetName("others");
  }
}

/*! \brief Find the gradient node from an add op */
const Node* FindGrad(const Node* add_node) {
  LTC_CHECK(add_node->op().op == at::aten::add) << "Expecting add op, but got " << add_node->ToString();
  auto op0 = add_node->operand(0).node;
  auto op1 = add_node->operand(1).node;
  LTC_CHECK((op0->op() == *ops::ltc_device_data) ^ (op1->op() == *ops::ltc_device_data)) 
    << "Exactly one operand of the add must be a device data node!";
  if (op0->op() == *ops::ltc_device_data)
    return op0;
  else
    return op1;
}
...
void MemModelComputationClient::AnnotateNodeType(const std::vector<const Node*>& params, 
  ...
  for (auto& i : node_info_) {
    ...
    /*
     * We used to find gradient and wgt tensors by looking at optimizer updates. Now we don't have optimizer. 
     * Notice that all wgt gradients are outputs (ROOT nodes), while all activation gradients are outputs but
     * with future uses. We can use this to identify wgt gradients:
     * - Layer name ends with ".bwd"
     * - Node is in output set
     * - Has zero use count
     * - To make it more robust, we only try to find grad if the output is generated by an add
     * This is the node that actually puts the gradient into the pre-allocated storage location. However, to
     * avoid confusion when parsing the JSON, we mark the pre-allocated storage (a device_data node) as grad
     * instead. 
     */
    auto layer_name = i.layer_name;
    auto outp = Output(node, 0);
    if ((layer_name.substr(layer_name.size()-4, 4) == ".bwd") && 
        (output_set.count(node)) && 
        (!use_cnts_.count(outp) || (use_cnts_.at(outp).size() == 0)) &&
        (node->op().op == at::aten::add)) {
      grads.insert(FindGrad(node));
    }
  }

  /* 
   * For weights, we find them from parameters by looking at the shapes of tensors. For each gradient,
   * we find a non-gradient tensor from the params set with the same shape, and mark that tensor as
   * weight. All other tensors left in the params set will be treated as inputs. Notice that from 
   * batch 2, LTC will not mark weight tensors as output but will still mark input tensors as output. 
   * This can help us better distinguish inputs and weights.  
   */
  for (auto grad_node : grads) {
    bool found = false;
    for (auto param_node : params_set) {
      if ((!output_set.count(param_node)) && 
          (!grads.count(param_node)) && 
          (!wgts.count(param_node)) && 
          (param_node->shape() == grad_node->shape())) {
        wgts.insert(param_node);
        found = true;
        break;
      }
    }
    if (!found) {
      LTC_LOG(INFO) << "[WARNING] The corresponding weight tensor for gradient " << grad_node->ToString()
                    << " is not found!";
    }
  }
