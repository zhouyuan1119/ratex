...
struct TensorInfo {
  ...
  TensorInfo(double size, int64_t uses, bool param, const torch_lazy_tensors::ir::Output orig_tensor) 
  ...
  bool is_view = false;
  const torch_lazy_tensors::ir::Output viewing;
  // If this tensor has multiple views, keep a set of pointers to each view
  torch_lazy_tensors::ir::OutputSet viewers = {};
};
...
class MemModelComputationClient : public BaseComputationClient {
 public:
  ...
  virtual std::vector<LayerMemInfo> GetMemoryBreakDown() override { return memory_breakdown_; }

 private:
  /*! \brief Peak memory maintained by this client. */
  double peak_memory_ = 0.0;
  /*! \brief Memory breakdown of each layer. */
  std::vector<LayerMemInfo> memory_breakdown_;
};
 ...
 */
std::vector<double> CalculateMemFromShape(const lazy_tensors::client::ShapeData& shape);
 ...
 * \param memory_breakdown Output, breakdown of layer-wise memory information. 
 */
