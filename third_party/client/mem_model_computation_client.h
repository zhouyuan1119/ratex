...
class MemModelComputationClient : public BaseComputationClient {
 ...
 private:
  ...
  /*! \brief Our internal use count tracker. Each (k, v) pair means tensor k has a use count of
   * v. Notice that we use ir::Output instead of ir::Node. ir::Output roughly represents a tensor in 
   * the program, where the op that generated the tensor (node) and the index of the tensor (index) 
   * are both recorded. The latter is useful if the tensor is part of a tuple. Tensors that have 
   * zero use count are not in the map. 
   */
  OutputMap<std::vector<const Node*>> use_cnts_;
